...
namespace BVHTools
{
    public class BVHAnimationLoader : MonoBehaviour
    {
        ...
        [Serializable]
        public struct StandardHumanoidBone
        {
            // Id bone
            public HumanBodyBones BoneIndex;
            public Transform TransformBone;
        }
        // The key is the bvhName
        private Dictionary<string, StandardHumanoidBone> retargetBonesMap;

        public void RetargetBonesMap()
        {
            retargetBonesMap = new Dictionary<string, StandardHumanoidBone>();
            if (targetAvatar != null)
            {
                for (int i = 0; i < ((int)HumanBodyBones.LastBone); i++)
                {
                    ...
                    // Include discarded boens
                    //Debug.Log("<color=cyan>" + "[RetargetBonesMap] Looking for: " + boneIndex.ToString() + "</color>");
                    Transform bone = targetAvatar.GetBoneTransform(boneIndex);

                    if (bone != null)
                    {
                        StandardHumanoidBone humanoidBone = new StandardHumanoidBone();
                        humanoidBone.BoneIndex = boneIndex;
                        humanoidBone.TransformBone = bone;
                        retargetBonesMap.Add(boneIndex.ToString(), humanoidBone);
                    }
                }


                boneRenamingMap = new FakeDictionary[(int)HumanBodyBones.LastBone - 1];

                for (int i = 0; i < ((int)HumanBodyBones.LastBone) - 1; i++)
                {
                    boneRenamingMap[i].targetName = string.Empty;
                    HumanBodyBones boneIndex = (HumanBodyBones)i;
                    Transform bone = targetAvatar.GetBoneTransform(boneIndex);
                    if (bone != null)
                    {
                        boneRenamingMap[i].targetName = bone.name;
                    }
                    boneRenamingMap[i].bvhName = boneIndex.ToString();
                }
        ...
        private Transform getBoneByName(string name, Transform transform, bool first)
        {
            ...
            for (int i = 0; i < transform.childCount; i++)
            {
                ...
                // BUG FIX FIND THE CHILD UNTIL IF IT REACHES THE END CHILD
            ...
            Debug.Log("<color=red>" + "Could not find bone \"" + name + "\" under bone \"" + transform.name + "\"." + "</color>");
            return null;
        ...
        { 
            ...
            // convert this node to bone humanoid
            if (retargetBonesMap != null)
            {
                if (retargetBonesMap.ContainsKey(node.name))
                {
                    nodeTransform = retargetBonesMap[node.name].TransformBone;
                }
                else
                {
                    Debug.Log("<color=red>" + "Node not Found: " + node.name + "</color>");
                }
            }
